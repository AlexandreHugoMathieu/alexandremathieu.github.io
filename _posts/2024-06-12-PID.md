...
The **Potential Induced Degradation (PID)** typically appears after the installation is commissioned and can reduce the performance of a PV string by 20-80%, according to Libby et al. [1]. 
...
Significant potential difference (several hundred volts) between the ground and the cell is the major cause of current leakage and the higher the voltage is, the more the PID degrades the module performances.
