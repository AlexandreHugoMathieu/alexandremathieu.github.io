...
As illustrated in Figure 1, the potential difference between PV modules and Cells increases with the number of modules in series. Then, PID, or Potential Induced Degradation, occurs when the PV system experiences significant voltage differences between the cell and the frame and leads to leakage currents.
 
<figure>
    <img src="https://alexandrehugomathieu.github.io/alexandremathieu.github.io//images/pid_images/pid_voltage.png">
    <figcaption> Figure 1: Illustration of the voltage in a PV string [4] </figcaption>
</figure>
...
High humidity and elevated temperature enables better conductivity and facilitate current leakage. Also, positive charges coming from soiling [2] can accelerate the degradation process.
- **Module Composition:**
...
To explain PID in a nutshell, the potential difference between the cell and the frame causes 6 leakage current paths as illustrated in Figure 2. The most significant leakage currents pass through path 1. This leakage particularly increases in humid conditions, due to a higher conductivity of the front glass plate surface. [2]
...
Mostly heterogeneous among the module population [4], the PID electrical signature is characterized by a decrease in the shunt resistance \( R_{sh} \)  and increase in junction recombination parameters \( I_{D2} \) and \( n_2 \) of the second diode in the two-diode model as illustrated in Figure 3.
...
With regard to the evolution over time, c-Si PID (excluding the regeneration process) is directly proportional to the applied voltage and has been modeled as in the equation below according to Hacke et al.'s model [6]
...
Visually detecting PID is challenging [7] and the failure recognition relies, then, on other detection methods such as electroluminescence, thermography, and UI tracing. Electroluminescence is the most spread and accurate technic to identify PID. IV tracing or direct Voc measurement are less accurate, but it can easily and quickly detect an anomaly from PID. Finally, infrared thermography will reveal the consequential hotspots from PIDs.
...
Electrical regeneration, on the other hand, utilizes the application of reverse voltage to restore the natural composition of the materials. For instance, the IEA report [7] illustrates a 94% regeneration after applying reverse voltage. However, electrical regeneration is never 100% complete due to the original induced corrosion + remaining Na in the "stacking" faults [2].

### VI. Prevention

When addressing potential-induced degradation (PID), some specific actions can significantly mitigate its impact, ensuring sustained efficiency over time. The target of those actions can be broken down into 4 categories as illustrated in Figure 5.
...
3.	Module: Enhance the material encapsulation by considering alternatives like TPO, ionomer, or polyolefin [1], [2]. Additionally, replacing the front glass with a lower ion concentration variant can avoid PID [2].
4.	Cell: Increase the Si/N ratio to reduce sodium ion storage in the ARC layer [1], [2]. Introducing a SiO2 layer between the Si cell and the ARC layer has also been demonstrated to reduce PID [2].
